...
class Client:
        ...
        if rawData[:4] in ["auth", "data", "resu"]:
            ...
            if rawData[:4] == "auth":
                ...
                if self.data['playerID'] == 1:
                    ...
                    self.P1.ready = self.data['playerReady']
                    ...
                    self.P1.ready = self.data['player2Ready']
                ...
                if self.player == 2:
                    self.P1.ready = self.data['P1'][0]
                    self.P1.currentCard = tuple(self.data['P1'][1])
                else:
                    self.P2.ready = self.data['P2'][0]
    ...
    def stop(self):
        ...
        print("[CLIENT] Disconnected from {self.host}:{self.port}.")
        self.sock.close()
        print("[CLIENT] Closed Connection. Initializing Exit...")
        sys.exit()
